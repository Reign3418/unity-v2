...
/**
 * GET: Fetches the active Team Builder layouts for a requested Kingdom.
 */
export async function GET(request) {
    try {
        ...
        if (result.Item && result.Item.data && result.Item.data.S) {
            ...
            // Return empty Team framework if none exists
            return new Response(JSON.stringify({ squads: [] }), { status: 200 });
        }

    } catch (e) {
        console.error("[AWS Teams] Fetch Error:", e);
        return new Response(JSON.stringify({ error: "Internal Database Sync Error" }), { status: 500 });
    }
}

/**
 * POST: Overwrites the active Team Builder array for a requested Kingdom.
...
export async function POST(request) {
    try {
        ...
        const body = await request.json(); // { squads }
        ...
        console.error("[AWS Teams] Dispatch Error:", e);
